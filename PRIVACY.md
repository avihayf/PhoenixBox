...
- **`history`**: Required to remove internal confirmation-page URLs from your browsing history so they don't clutter the address bar.
...
1. **User-Agent List Updates**: The extension fetches a list of top user agents from `https://cdn.jsdelivr.net` (the microlinkhq/top-user-agents CDN) when you use the User-Agent override feature. This list is cached locally for 7 days. No browsing history or container data is transmitted during this request.

This connection is explicitly allowed in the Content Security Policy and can be disabled by not using the User-Agent override feature.

2. **Optional Firefox Sync**: If you explicitly enable sync, Phoenix Box stores supported configuration data in `browser.storage.sync`, which is tied to your Mozilla account. Passwords are not synced.

## Third-Party Services

Phoenix Box does not integrate with any third-party analytics, advertising, or tracking services.
...
This is intentional for security testing workflows. The Burp Suite extension (Phoenix-Highlighter.jar) is designed to strip this header before forwarding requests to prevent fingerprinting.
...
- Open an issue on [GitHub](https://github.com/avihaife-cmyk/PhoenixBox/issues)
- Contact the developer through the GitHub profile

## Open Source

Phoenix Box is open source under the Mozilla Public License 2.0. You can review the complete source code at [https://github.com/avihaife-cmyk/PhoenixBox](https://github.com/avihaife-cmyk/PhoenixBox).

---

**Summary**: Phoenix Box does not collect telemetry or track users. It operates locally, with optional jsDelivr User-Agent updates and optional Firefox Sync when you choose to enable those features.
